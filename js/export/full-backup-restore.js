...
export function createRestorePreview(backup) {
  ...
  return { appVersion: backup.appVersion, schemaVersion: backup.schemaVersion, createdAt: backup.createdAt, sourceDeviceLabel: backup.sourceDeviceLabel, counts: backup.sectionCounts, symbols, firstDate: dates[0] || null, lastDate: dates.at(-1) || null, warnings };
}
...
function validateRecords(name, records) {
  ...
  records.forEach((record) => {
    ...
    const key = name === STORE_NAMES.HISTORICAL_CANDLES ? `${record.symbol}|${record.date}` : record.id ?? record.symbol;
    if (!key || keys.has(key)) throw new Error(`Backup section ${name} contains duplicate keys.`);
    keys.add(key);
    if (name === STORE_NAMES.HISTORICAL_CANDLES && (!validSymbol(record.symbol) || !/^\d{4}-\d{2}-\d{2}$/.test(record.date || '') || !Number.isFinite(Number(record.close)))) throw new Error('Backup contains an invalid historical candle.');
  });
}
...
function validateSectionCounts(backup) {
  ...
  const expected = {
    holdings: backup.settings.holdings.length, lots: backup.settings.lots.length, benchmarks: backup.settings.benchmarks.length,
